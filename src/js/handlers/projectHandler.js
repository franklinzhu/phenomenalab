...
Projects.prototype.init = function() {
  ...
  if (this.md.mobile()) {
    this.camera.position.set(0, 0, 4800);
  } else {
...
Projects.prototype.render = function() {
  if (this.md.mobile()) {
    if (this.betaRotation < -180) {
      this.bR = 90;
    } else if (this.betaRotation < -89.99) {
      this.bR = -90;
    } else {
      this.bR = this.betaRotation;
    }

    this.camera.position.y += (this.bR * 25 - this.camera.position.y) * 0.02;
    this.camera.position.x +=
      (-this.alphaRotation * 20 - this.camera.position.x) * 0.02;
...
Projects.prototype.deviceorientation = function(e) {
  this.alphaRotation = e.alpha ? e.alpha : 0;
  this.betaRotation = (e.beta ? e.beta : 0) - 90;
};
